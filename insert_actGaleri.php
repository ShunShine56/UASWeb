...
if ($foto_size > 2097152) {
	...
	header("location:insertGaleri.php?pesan=size");
}else{
	...
	if ($foto_nama != "") {
		...
		if(in_array($ekstensi, $ekstensi_izin) === true)  {
            ...
            $query = mysqli_query($koneksi, "INSERT INTO galeri VALUES ('','$foto_nama','$judul_gambar','$deskripsi','$tanggal')");

            // Mengecek apakah data gagal diinput atau tidak
            if($query){
            	header("location:insertGaleri.php?pesan=berhasil");
            } else {
                header("location:insertGaleri.php?pesan=gagal");
            }

        } else { 
        	// Jika ekstensinya tidak sesuai dengan apa yg kita tetapkan maka error
        	header("location:insertGaleri.php?pesan=ekstensi");       }

    }else{

    	// Apabila tidak ada file yang diupload maka akan menjalankan code dibawah ini
    	 $query = mysqli_query($koneksi, "INSERT INTO galeri(judul_gambar, tanggal, deskripsi) VALUES ('$judul_gambar', '$tanggal', '$deskripsi')");
