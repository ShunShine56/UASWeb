...
if ($foto_size > 2097152) {
	...
	header("location:insertSambutan.php?pesan=size");
}else{
	...
	if ($foto_nama != "") {
		...
		if(in_array($ekstensi, $ekstensi_izin) === true)  {
            ...
            $query = mysqli_query($koneksi, "INSERT INTO kepalasekolah VALUES ('','$foto_nama', '$nama_kepsek', '$sambutan')");

            // Mengecek apakah data gagal diinput atau tidak
            if($query){
            	header("location:insertSambutan.php?pesan=berhasil");
            } else {
                header("location:insertSambutan.php?pesan=gagal");
            }

        } else { 
        	// Jika ekstensinya tidak sesuai dengan apa yg kita tetapkan maka error
        	header("location:insertSambutan.php?pesan=ekstensi");       }

    }else{

    	// Apabila tidak ada file yang diupload maka akan menjalankan code dibawah ini
    	 $query = mysqli_query($koneksi, "INSERT INTO kepalasekolah(nama_kepsek, sambutan) VALUES ('$nama_kepsek', '$sambutan')");
