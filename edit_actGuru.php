...
if (isset($_POST['id'])) {
    ...
    if ($foto_size > 2097152) {
       ...
       header("location:indexGuruKaryawan.php?pesan=size");
       ...
       if ($foto_nama != "") {
          ...
          if(in_array($ekstensi, $ekstensi_izin) === true)  {
            ...
            $query = mysqli_query($koneksi, "UPDATE gurukaryawan SET Gambar='$foto_nama', nama_guru='$nama_guru', keterangan='$keterangan'  WHERE id ='$id'");

            // Mengecek apakah data gagal diinput atau tidak
            if($query){
            	header("location:indexGuruKaryawan.php?pesan=berhasil");
            } else {
                header("location:indexGuruKaryawan.php?pesan=gagal");
            }

        } else { 
        	// Jika ekstensinya tidak sesuai dengan apa yg kita tetapkan maka error
        	header("location:indexGuruKaryawan.php?pesan=ekstensi");        }

        }else{

    	// Apabila tidak ada file yang diupload maka akan menjalankan code dibawah ini
          $query = mysqli_query($koneksi, "UPDATE gurukaryawan SET nama_guru='$nama_guru', keterangan='$keterangan' WHERE id='$id'");
